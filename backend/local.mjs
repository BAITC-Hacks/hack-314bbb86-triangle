...
const server=http.createServer(async(req,res)=>{
 try {
  ...
  if(url.pathname.startsWith('/api/')){
   const cookie=req.headers.cookie?.match(/(?:^|; )akim_local=([a-f0-9-]{36})(?:;|$)/)?.[1]||randomUUID();
   const identity={id:`local:${cookie}`,name:'Local player'};
   const request=new Request(url,{method:req.method,headers:req.headers,...(!['GET','HEAD'].includes(req.method)?{body:req,duplex:'half'}:{})});
   const response=await api(request,{...process.env,DB:db,LOCAL:'1'},identity);
   res.writeHead(response.status,{...Object.fromEntries(response.headers),'Set-Cookie':`akim_local=${cookie}; HttpOnly; SameSite=Lax; Path=/; Max-Age=2592000`});res.end(Buffer.from(await response.arrayBuffer()));return;
  }
