...
export function factsFor(decisions,result,locale) {
 ...
 return {baseline:BASELINE.score,result:localizedResult,recommendations:recommend(decisions).map(r=>({...r,removeName:copy.measures[r.remove.measureId].name,addName:copy.measures[r.add.measureId].name,addDistrict:r.add.districtId?copy.districts[r.add.districtId].name:pick(locale,'Бүкіл қала','Весь город','Entire city')})),
  decisions:decisions.map(d=>({...d,...MEASURES.find(m=>m.id===d.measureId),...copy.measures[d.measureId],district:d.districtId?copy.districts[d.districtId].name:pick(locale,'Бүкіл қала','Весь город','Entire city')}))};
}
export function localReport(facts,locale) {
 ...
 return {summary:pick(locale,`Сценарий қаланың баллын ${n(r.delta)} ұпайға жақсартады.`,`Сценарий улучшает городской балл на ${n(r.delta)}.`,`The scenario improves the city score by ${n(r.delta)} points.`),
...
export async function explain(facts,locale,env,fetcher=fetch) {
 ...
 return {mode:'ai',model:env.OPENAI_MODEL||'gpt-4.1-mini',report};
