...
export function calculate(decisions) {
  ...
  for (const d of decisions) {
    const m = measures.get(d.measureId), factor = (HORIZON - m.lag) / HORIZON;
  ...
  for (const s of SYNERGIES) {
    ...
    if (first && decisions.some(d => d.measureId === s.pair[1])) {
...
export function simulate(decisions) {
  ...
  return { valid: true, errors: [], ...result, delta: result.score - BASELINE.score, contributions };
}
